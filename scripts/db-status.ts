...
try {
  ...
  try {
    ...
    if (status.latestOneShotRun) console.log(`latest_one_shot_run=${status.latestOneShotRun.id} status=${status.latestOneShotRun.status} transport=${status.latestOneShotRun.selectedTransport ?? "legacy_http"} permission=${status.latestOneShotRun.permissionStatus ?? "n/a"} started=${status.latestOneShotRun.startedAt}`);
