...
export function listRecentCollectionRuns(database: Database.Database, limit = 10): RecentCollectionRun[] {
  ...
  const rows = database.prepare(`SELECT r.id, r.started_at, r.completed_at, r.status, r.selected_detail_count,
    r.inserted_count, r.updated_count, r.unchanged_count, r.skipped_count, r.failed_count,
    ...
    WHERE r.source = 'jobkorea' AND r.dry_run = 0
  ...
  return rows.map((row) => {
    ...
    return { id: String(row.id), startedAt, completedAt, presetId: typeof row.preset_id === "string" ? row.preset_id : null,
      presetLabel: typeof row.preset_label === "string" ? row.preset_label : "잡코리아 수동 수집",
