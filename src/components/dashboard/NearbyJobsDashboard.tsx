...
export function NearbyJobsDashboard({ initialJobs, dataError }: NearbyJobsDashboardProps) {
  ...
  const [selectedJobId, setSelectedJobId] = useState<string | null>(initialJobs[0]?.job.id ?? null);
  ...
  return (
    <main className="app-shell">
      <AppHeader filters={filters} mapVisible={mapVisible} onFiltersChange={setFilters} onToggleFilters={() => setFiltersOpen(true)}
        onToggleMap={() => { setMapVisible((value) => !value); if (!mapVisible) setMobileView("map"); }} />
      ...
      <div className="mobile-view-switch" aria-label="모바일 화면 전환">
        <button type="button" className={mobileView === "list" ? "active" : ""} onClick={() => setMobileView("list")} aria-pressed={mobileView === "list"}>목록</button>
        <button type="button" className={mobileView === "map" ? "active" : ""} onClick={() => { setMobileView("map"); setMapVisible(true); }} aria-pressed={mobileView === "map"}>지도</button>
      </div>
      ...
      {dataError ? <div className="state-panel" role="alert"><h2>데이터를 표시할 수 없습니다</h2><p>{dataError}</p></div>
        ...
        : <div className={`dashboard-body ${mapVisible ? "" : "map-hidden"}`}>
          <section className={`list-panel ${mobileView === "map" ? "mobile-hidden" : ""}`} aria-label="통합 공고 목록 패널">
