...
import { LocalHelpPanel } from "../help/LocalHelpPanel";
...
export function FirstRunOnboarding({readiness={version:"0.1.1",databaseReady:true,migrationsReady:true,chromiumReady:false,collectionUiEnabled:false,localhostSafe:true,latestBackupAvailable:false},forceOpen=false,onClose}:{readiness?:LocalReadiness;forceOpen?:boolean;onClose?():void}){const [open,setOpen]=useState(forceOpen);const [step,setStep]=useState(0);const ref=useRef<HTMLDivElement>(null);
 ...
 const finish=(value:"completed"|"dismissed")=>{try{window.localStorage.setItem(ONBOARDING_STORAGE_KEY,value);}catch{/* storage can be disabled */}setOpen(false);onClose?.();};
 ...
 if(!open)return null;const current=STEPS[step]!;return <div className="onboarding-backdrop"><section ref={ref} className="onboarding-dialog" role="dialog" aria-modal="true" aria-labelledby="onboarding-title" onKeyDown={key}><div className="onboarding-progress" aria-label={`온보딩 ${step+1}/${STEPS.length}`}><progress max={STEPS.length} value={step+1}/><span>{step+1} / {STEPS.length}</span></div><h2 id="onboarding-title">{current.title}</h2><p>{current.body}</p>{step===1&&<div className="onboarding-labels"><span>데모</span><span>픽스처</span><span>수동 수집</span><span>목록 정보</span><span>상세 확인</span></div>}{step===3&&<p className="safe-notice">예: 서울·경기만 선택하고 강사·웨이터 같은 키워드를 제외할 수 있습니다.</p>}{step===STEPS.length-1&&<LocalHelpPanel readiness={readiness} compact/>}<div className="onboarding-actions">{step>0&&<button className="button soft" onClick={()=>setStep(step-1)}>이전</button>}{step<STEPS.length-1?<button className="button primary" onClick={()=>setStep(step+1)}>{step===0?"시작하기":"다음"}</button>:<><button className="button primary" onClick={()=>finish("completed")}>공고 둘러보기</button><Link className="button soft collection-nav-link" href="/collection" onClick={()=>finish("completed")}>수집관리로 이동</Link></>}<button className="button" onClick={()=>finish("dismissed")}>{step===0?"건너뛰기":"다시 보지 않기"}</button></div></section></div>}
