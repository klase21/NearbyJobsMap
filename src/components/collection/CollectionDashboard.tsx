...
export function CollectionDashboard({ enabled, presets }: Props) {
  ...
  return <div className="collection-dashboard-shell">
    ...
    {view === "overview" ? <section id="overview-panel" role="tabpanel" aria-labelledby="overview-tab" aria-busy={loading}>
      ...
      <ManualBackfillPanel enabled={enabled} onWriteCompleted={handleWriteCompleted} />
    </section>}
  </div>;
}
...
function CollectionOverview({ dashboard, filters, loading, error, onFilters, onRefresh }: { dashboard: CollectionDashboardData | null; filters: CollectionDashboardFilters; loading: boolean; error: string | null; onFilters(value: CollectionDashboardFilters): void; onRefresh(): Promise<void> }) {
  ...
  return <div className="collection-overview">
    ...
    {dashboard && <>
      <DashboardSection title="오늘 수집" description={`${dashboard.todayCollection.localDate} · Asia/Seoul 등록 공고만 집계합니다.`}><MetricGrid items={[["잡코리아 신규",dashboard.todayCollection.jobkorea.inserted],["잡코리아 갱신",dashboard.todayCollection.jobkorea.updated],["알바몬 신규",dashboard.todayCollection.albamon.inserted],["알바몬 갱신",dashboard.todayCollection.albamon.updated],["동일",dashboard.todayCollection.unchanged],["제외",dashboard.todayCollection.excluded],["소스 실패",dashboard.todayCollection.sourceFailures],["최근 실행",dateOrUnknown(dashboard.todayCollection.lastRunAt)]]}/></DashboardSection>
      ...
      <div className="dashboard-two-column"><DashboardSection title="지역 범위"><CoverageList entries={[["서울", dashboard.regions.seoul], ["경기", dashboard.regions.gyeonggi], ["수도권 범위만 확인", dashboard.regions.capitalScope], ["복수 지역", dashboard.regions.multiple], ["기타", dashboard.regions.other], ["지역 미확인", dashboard.regions.unknown]]} /></DashboardSection>
      ...
      <DashboardSection title="데이터 품질" description="통근 준비는 신뢰 가능한 좌표 또는 전체 주소가 있는 공고만 보수적으로 집계합니다. 경로·통근비 계산은 아직 제공하지 않습니다."><MetricGrid items={[
        ["전체 주소", dashboard.dataQuality.address.full_address], ["시·군·구", dashboard.dataQuality.address.city_district], ["지역만", dashboard.dataQuality.address.region_only], ["복수 근무지", dashboard.dataQuality.address.multiple_locations], ["위치 미확인", dashboard.dataQuality.address.unknown], ["오염 위치", dashboard.dataQuality.address.contaminated],
        ["구조화 급여", dashboard.dataQuality.salary.structured], ["표시 급여만", dashboard.dataQuality.salary.display_only], ["급여 협의", dashboard.dataQuality.salary.negotiable], ["급여 미확인", dashboard.dataQuality.salary.unknown], ["잘못된 급여", dashboard.dataQuality.salary.invalid], ["좌표 있음", dashboard.dataQuality.coordinateRecords], ["통근 준비", dashboard.dataQuality.commuteReadyRecords], ["수도권 범위만 확인", dashboard.dataQuality.capitalScopeOnlyRecords]
      ]} /></DashboardSection>
