...
describe("SQLite migration", () => {
  it("빈 데이터베이스에 초기 migration을 적용하고 버전을 기록한다", () => {
    ...
    expect(result.applied).toEqual(["0001", "0002", "0003", "0004", "0005", "0006", "0007"]);
    expect(listAppliedMigrations(testDatabase.database)).toEqual(["0001", "0002", "0003", "0004", "0005", "0006", "0007"]);
  ...
  it("반복 실행은 schema를 다시 적용하지 않는다", () => {
    ...
    expect(applyMigrations(testDatabase.database)).toMatchObject({ applied: [], alreadyApplied: ["0001", "0002", "0003", "0004", "0005", "0006", "0007"] });
  });
