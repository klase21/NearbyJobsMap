...
export interface SavedJobView { id:string;name:string;filters:JobFilterState;isFavorite:boolean;isDefault:boolean;createdAt:string;updatedAt:string;lastUsedAt:string|null }
export interface SavedJobViewInput { name:string;filters:JobFilterState;isFavorite?:boolean;isDefault?:boolean }
...
export function validateSavedJobViewInput(raw:unknown):SavedJobViewInput {
  ...
  const record=raw as Record<string,unknown>;const allowed=new Set(["name","filters","isFavorite","isDefault"]);
  ...
  return{name,filters:record.filters,isFavorite:record.isFavorite===true,isDefault:record.isDefault===true};
}
