...
이 shadow 진단 단계에서는 실제 잡코리아 요청, 상세 navigation, page 2, `_GI_List`, retry, SQLite write를 한 번도 실행하지 않았다. 다음 정확한 작업은 **별도 승인된 page-1, `max-details=0`, schema-2 Playwright dry-run 1회로 provisional structural group을 측정하는 것**이다. 그 결과가 실제 반복 single-posting group을 보여주기 전에는 ordinary selector를 변경하지 않는다.
